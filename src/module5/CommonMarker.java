...
public abstract class CommonMarker extends SimplePointMarker {
	...
	public void draw(PGraphics pg, float x, float y) {
		...
		if (!hidden) {
			drawMarker(pg, x, y);
//oh gosh, i commented that as i found better way to draw titles(in EarthquakeCityMap) - or not ;) but it works...
			/*if (selected) {
				showTitle(pg, x, y);
				// You will implement this in the subclasses
			}*/
		}
	}
	
	public void title(PGraphics buff, float x, float y, String title) {
		buff.beginDraw();
		int buffer = 20;
		int length = title.length();
		buff.fill(255);
		buff.rect(x-radius, y+2*radius, 7*length, buffer);
		buff.fill(0);
		buff.text(title, x, y+3*radius);
		buff.endDraw();
